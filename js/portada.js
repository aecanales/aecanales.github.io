...
var initial_x = -100; //traslación x inicial
var dx = 5; //espacio horizontal entre mujeres
var initial_y = -55; //traslación y inicial
var dy = 7; //espacion vertical entre mujeres
...
window.onload = function() {
	$( "#database" ).load( "data/database_main.csv", function() {
		...
		for (var i = 41; i < 81; i++) {
			createFemale(initial_x + dx*(i-40), initial_y + dy, database[i])
		}
		for (var i = 81; i < 121; i++) {
			createFemale(initial_x + dx*(i-80), initial_y + dy*2, database[i])
		}

		for (var i = 121; i < 161; i++) {
			createFemale(initial_x + dx*(i-120), initial_y + dy*3, database[i])
		}

		for (var i = 161; i < 201; i++) {
			createFemale(initial_x + dx*(i-160), initial_y + dy*4, database[i])
		}

		for (var i = 201; i < 241; i++) {
			createFemale(initial_x + dx*(i-200), initial_y + dy*12, database[i])
		}
		for (var i = 241; i < 281; i++) {
			createFemale(initial_x + dx*(i-240), initial_y + dy*13, database[i])
		}
		for (var i = 281; i < 321; i++) {
			createFemale(initial_x + dx*(i-280), initial_y + dy*14, database[i])
		}
		for (var i = 321; i < 361; i++) {
			createFemale(initial_x + dx*(i-320), initial_y + dy*15, database[i])
		}
		for (var i = 361; i < 401; i++) {
			createFemale(initial_x + dx*(i-360), initial_y + dy*16, database[i])
		}
